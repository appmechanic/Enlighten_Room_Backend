...
// into the new lesson. Classwork tagged with prior lesson names is preserved
// so per-lesson reports (viewAllAnswers, getStudentLessonReport) can still
// render historical data. Also persists a Lesson document so we can track
// the active lesson per room and keep a history for reporting.
export const startLessonForRoom = async (req, res) => {
  try {
    ...
    // Adopt any pre-staged questions (lessonName === '') into the new lesson.
    // Prior-lesson questions are left untouched so their reports survive.
  ...
  try {
    ...
    if (ended) {
      ...
      if (isFreshByTime && submissionCountMatches) {
        ...
        // Fire-and-forget. If this errors (Gemini 503, retry exhaustion,
        // server restart mid-flight) the lesson is left with an empty
        // classReport — but the read-side self-heal
        // (scheduleMissingReportBackfills) will retry the next time a
        // teacher or student opens the report, so the failure is not
        // permanent. Logs stay loud enough to grep for the lesson id.
        generateAndStoreClassReport(ended).catch((err) => {
          console.error(
            `[endLessonForRoom] background class report failed for lesson=${String(ended._id)} name="${ended.name}" roomId=${ended.roomId}. Will retry on next report read.`,
            err?.message || err,
          );
        });
      }
...
// In-memory dedup set: lesson IDs currently being (re)generated by the
// on-read self-healing path. Prevents the same lesson from being fired at
// Gemini multiple times per second when a teacher rapidly refreshes the
...
function scheduleMissingReportBackfills(lessonDocs) {
  ...
  for (const l of lessonDocs) {
    ...
    Lesson.findById(l._id)
      .then((full) => {
        if (!full) return null;
        return generateAndStoreClassReport(full);
      })
      .catch((err) => {
        console.error(
          `[ClassReport] backfill failed for lesson=${key} — will retry on next report read.`,
          err?.message || err,
        );
      })
      .finally(() => {
...
export const getStudentLessonReport = async (req, res) => {
  try {
    ...
    // Self-heal: any ended lesson missing a class report gets a background
    // regen. The current response still returns the null classReport, but
    // the next student visit will see the populated one.
    scheduleMissingReportBackfills(lessons);
    ...
    const projectQuestion = (q) => {
      ...
      return {
        ...
        submission: submission
          ...
          : null,
        // Student is viewing their own report — surface their own submitted
        // answer per attempt so the "You answered" line renders instead of "—".
        aiReport: aiReport ? projectAiReport(aiReport, { includeStudentAnswer: true }) : null,
      };
    };
